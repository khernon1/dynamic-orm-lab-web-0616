...
class InteractiveRecord
  ...
  def self.find_by(options={})
    #need to iterate through the entire db until we find the value passed 
    #split the options into key and value pair
    sql = <<-SQL
      SELECT * FROM #{self.table_name} WHERE #{options.keys.join("")} = '#{options.values.join("")}'
      SQL
       #binding.pry
